...
\begin{scope}[x=2.4cm,y=1.2cm, shift={(1.,0.5)}]
  \readlist\Nnod{2,3,5,3}
  \readlist\Nstr{l,n_1,{}}
  \readlist\Nnstr{l,n_L,n}
...
\node[right=0.5cm of N4-2, font=\large, anchor=west, yshift=-0.4cm] (matrix) {$\begin{bmatrix} \frac{dx_1}{dt} \\[2em] \frac{dx_2}{dt} \\[1em] \vdots\\[1em] \frac{dx_n}{dt} \end{bmatrix} \in \mathbb{R}^{L_s \times 1}$};
\begin{pgfonlayer}{background}
  \node[draw=black, dashed, rounded corners=0.3cm, 
        fit=(N1-1) (N1-2) (N2-1) (N2-3) (N3-1) (N3-5) (N4-1) (L1) (L2) (L3) (L4) (matrix), 
        inner sep=0.1cm, 
  ...
  \node[above=0.3,align=center,mydarkred, anchor=west] (L4) at (ann_box.south west) {ANN  Network (ANN-$f$)};
\end{pgfonlayer}

\end{scope}


\coordinate (mypoint) at (N1-2);



% ===== ODE SOLVER BOX =====
\node[box, label] (ode) at ([xshift=6cm]matrix.west) {ODE\\solver};
% ===== Input arrows =====
\draw[arrow] (N1-1)++(-4,0) -- (N1-1) node[midway, above, label] {$\mathbf{x}(t)$};
\draw[arrow] (N1-2)++(-4,0) -- (N1-2) node[midway, below, label] {$v(t)$};

% ===== Connection between NN output and ODE solver =====
\draw[arrow] (matrix.east) -- (ode.west) node[midway, above, label] {${f}(\mathbf{x},v)$};


% ===== Dashed feedback line =====
\draw[dashed_arrow, very thick] let \p1=(N1-1) in 
      (ode.north) to[out=90, in=90, looseness=1] (\x1-3cm,\y1+0.2cm) coordinate (arrow_end1);

\node[circle, fill=black, inner sep=1.5pt, yshift=-0.2cm] at (arrow_end1) {};
...
\node[label] at (10, 6) {$\mathbf{x}(t + \Delta t)$};



\begin{scope}[x=-2.4cm,y=1.2cm, shift={(-7.5,-6.5)}]
  \readlist\Nnod{1,3,5,1}
  \readlist\Nstr{l,n_1,{}}
  \readlist\Nnstr{l,n_L,{}}
  \readlist\Cstr{{},h^{(1)},h^{(l)}, {\G}}
  \def\yshift{0.55}
...
\begin{pgfonlayer}{background}
  \node[draw=black, dashed, rounded corners=0.3cm, 
        fit=(N1-1) (N2-1) (N2-3) (N3-1) (N3-5) (N4-1) (L1) (L2) (L3) (L4) ,
        inner sep=0.1cm, 
        label={[font=\Large]above:ANN},
        draw=mygreen, 
        fill = mygreen!10] (ann_box) {};
        

  \node[above=0.4,align=center,mydarkred, anchor=west] (L4) at (ann_box.south west) {ANN Network \\ (ANN-\(\G\))};
\end{pgfonlayer}
...
% ===== Output arrow =====
\draw[arrow] (ode.south) |- (N1-1) node[midway, below, label] (end) {$\mathbf{x}(t + \Delta t)$};


% ===== ODE SOLVER BOX =====
\node[box, label] (ode) at ([xshift=-3cm]N4-1) {\Large $\displaystyle v= h(\G, v_s(t))$};
\draw[arrow] (N4-1) -- (ode.east) node[midway, above, label] {};


\draw[dashed_arrow, very thick] let \p1=(mypoint) in 
      (ode.west) to[out=180, in=-90, looseness=1] (\x1-3cm,\y1-0.2cm) coordinate (arrow_end);
\node[circle, fill=black, inner sep=1.5pt, yshift=0.2cm] at (arrow_end) {};
