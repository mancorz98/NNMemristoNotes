...
\node[box, label, draw=mygreen, fill=mygreen!10, anchor=north east] (g_func) at  ([yshift=-2cm]matrix.south west) {\Large $\displaystyle \G = g\Large(\mathbf{x}(t)\Large)$};
...
% ===== Output arrow =====
\draw[arrow] (ode.south) |- (g_func.east) node[midway, below, label] (end) {$\mathbf{x}(t + \Delta t)$};


% ===== ODE SOLVER BOX =====
\node[box, label] (ode) at ([xshift=-3cm]g_func.west) {\Large $\displaystyle v= h(\G, v_s(t))$};
\draw[arrow] (g_func.west) -- (ode.east) node[midway, above, label] {};
