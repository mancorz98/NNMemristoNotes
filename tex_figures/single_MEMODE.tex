...
% ===== Output arrow =====
\draw[arrow] (ode.south) |- ++(-1, -4) node[midway, below, label] (end) {$x(t + \Delta t)$}
      -- ++(-2, 0)  node[box, label, anchor=east] (gfunc) {\Large $\displaystyle \G(x) = \frac{x}{R_{\text{ON}}} + \frac{1 - x}{R_{\text{OFF}}}$};

\draw[arrow] (gfunc.west) -- ++(-3,0) node[midway, above, label] {$\Gm(t + \Delta t)$}
      node[box, label, anchor=east] (vm) {\Large $\displaystyle v = h(\G, v_s(t))$};
...
\draw[dashed_arrow, very thick] let \p1=(mypoint) in 
      (vm.west) to[out=180, in=-90, looseness=1] (\x1-3cm,\y1-0.2cm) coordinate (arrow_end);
...
\draw[dashed_arrow, very thick] let \p1=(N1-1) in 
      (ode.north) to[out=90, in=90, looseness=1.1] (\x1-3cm,\y1+5) coordinate (arrow_end);
