...
\begin{figure}[!t]
  ...
  \vspace{-0.5in}
  \caption{Schematic diagram of the measurement setup used for the SDC memristor. The device is connected in series with a resistor, and the input voltage is applied via an arbitrary waveform generator. Voltage measurements are acquired using a data acquisition (\changed[approved]{myDAQ University Kit}) system.}
...
\changed[approved]{Their values are calculated during each optimization step using the the adaptive loss balancing procedure described in Algorithm~\ref{alg:adaptive_loss_balancing}~\cite{Cipolla2018, Chen2018}.}
...
The choice of the normalized mean squared error ($\NMSE$) as the primary metric is motivated by the substantial amplitude disparity between measured voltage and current signals. In the experimental dataset, the voltage amplitudes span 0.5--1.5~$\mathrm{V}$, whereas the corresponding current amplitudes range from approximately \(50~\mathrm{\mu A}\) to \(150~\mathrm{\mu A}\), resulting in a scale difference of up to three orders of magnitude. If raw MSE were employed as the optimization objective, the voltage error term would dominate the loss landscape by several orders of magnitude, effectively rendering the current fitting contribution negligible during gradient-based optimization. \changed[approved]{This would lead to models that accurately reproduce voltage waveforms while exhibiting poor current prediction, which is the quantity most relevant for characterizing memristive switching behavior.} The trajectory-specific normalization by ($\sigma_{z,k}$) ensures that both voltage and current channels contribute comparably to the optimization objective regardless of their absolute amplitudes, and provides a dimensionless metric with an intuitive interpretation: the ($\NMSE$) value directly quantifies the prediction error relative to the signal's intrinsic variability within each operating condition. 
...
\changed[approved]{We note that all experimental data used in this study were acquired from a single physical SDC device. While the proposed Neural ODE training pipeline---including adjoint sensitivity-based optimization and the multi-component loss function---is formulated in a device-agnostic manner, its applicability to other memristor technologies (e.g., HfO\textsubscript{2}-based RRAM, phase-change memory) and across inter-device variability remains to be validated and constitutes an important direction for future work.
