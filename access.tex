...
The experimental setup, illustrated in Fig.~\ref{fig:memristor_setup}, consists of an SDC memristor connected in series with a resistor. The sinusoidal input voltage is supplied by an arbitrary waveform generator. Voltage signals are measured using a data acquisition (\changed{myDAQ University Kit}) system. \changed{The series resistance is $\Rs = 5.11~\mathrm{k\Omega}$.
}

Measurements are carried out for the following combinations of supply voltage amplitudes and frequencies: \(\Vs  \in \{0.5,\ 1.0,\ 1.5\}~[{\rm V}]\), \(f \in \{1,\ 5,\ 10,\ 20,\ 50,\ 100\}~[\rm Hz]\). \changed{
...
\begin{figure}[!t]
  ...
  \vspace{-0.5in}
  \caption{Schematic diagram of the measurement setup used for the SDC memristor. The device is connected in series with a resistor, and the input voltage is applied via an arbitrary waveform generator. Voltage measurements are acquired using a data acquisition (\changed{myDAQ University Kit}) system.}
...
\changed{Their values are calculated during each optimization step using the the adaptive loss balancing procedure described in Algorithm~\ref{alg:adaptive_loss_balancing}~\cite{Cipolla2018, Chen2018}.}
...
\changed{We note that all experimental data used in this study were acquired from a single physical SDC device. While the proposed Neural ODE training pipeline---including adjoint sensitivity-based optimization and the multi-component loss function---is formulated in a device-agnostic manner, its applicability to other memristor technologies (e.g., HfO\textsubscript{2}-based RRAM, phase-change memory) and across inter-device variability remains to be validated and constitutes an important direction for future work.
