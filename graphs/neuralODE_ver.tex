\begin{tikzpicture}[
    ...
    \node[layer, minimum height=1.2cm, below=0.5cm of act1] (h2) {Hidden \changed{Layers $2,3,\ldots,L\!-\!1$}\\$(N)$};
    \node[activation, below=0.5cm of h2] (act2) {activation};
    %ZG replaced "L" by "$L$", replaced($\lfloor N/2 \rfloor$) by $(\lfloor N/2 \rfloor)$
    \node[layer, minimum height=1.2cm, below=0.5cm of act2] (h3) {Hidden Layer \changed{$L$}\\$(\lfloor N/2 \rfloor)$};
