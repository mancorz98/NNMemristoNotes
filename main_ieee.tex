...
\begin{figure}[!t]
  ...
  \caption{Schematic diagram of the measurement setup used for the SDC memristor. The device is connected in series with a resistor, and the input voltage is applied via an arbitrary waveform generator. Voltage measurements are acquired using a data acquisition (DAQ) system.
    ...
    \TODO{\(f_s\) should be replaced by \(f\)}
  }
...
The normalized mean squared error between two sequences {\color{red} \(({z}_{k,\sind=})_{\sind==1}^{\snum}\) and \((\tilde{z}_{k,\sind=})_{\sind==1}^{\snum}\) of \(\snum\)} \changed{\(({z}_{k,\sind})_{\sind=1}^{\snum}\) and \((\tilde{z}_{k,\sind})_{\sind=1}^{\snum}\) of \(\snum\)} data points is defined as
\begin{equation}
...
\begin{figure}[htbp]
  ...
  \caption{The \MemODETwo\ approach. The the artificial neural network (ANN) evaluates the function \(f(\mathbf{x},v)\), which is then integrated by the ODE solver to predict the state at the next time step \(\mathbf{x}(t\!+\!\Delta t)\). The second ANN models the memristor conductance \(\Gm(\mathbf{x})\) based on the next state \(\mathbf{x}(t\!+\!\Delta t)\). \changed{
   \changed{
    The function \(h(G)\) computes the memristor voltage \(\vm\) given the conductance \(G\).
  }
    }
...
In the \MemODEOne\ approach, the memristor conductance is defined on the basis of the physical mechanisms underlying self-directed channel (SDC) devices. In such memristors, the resistance evolves as a result of \(\mathrm{Ag^+}\) ions migration and the formation of conductive filaments. Consequently, the device conductance exhibits a continuous transition between different resistance states. This transition can be described as a weighted combination of the limiting resistance values, modulated by the internal variable \(x\)
\begin{equation}
  \Gm(x) = \frac{x}{R_{\rm ON}} + \frac{1 - x}{R_{\rm OFF}},
  \label{eq:conductance}
\end{equation}
where \(\Ron\) and \(\Roff\) denote the resistances in the low-conductance and high-conductance states, respectively. During the optimization process, the network parameters (weights \(w_{k,i}\) and biases \(b_k\)) are tuned jointly with the values of \(R_{\rm ON}\) and \(R_{\rm OFF}\). For this approach the following constraints are applied:
\begin{itemize}
...
In the \MemODETwo approach, the memristor conductance is modeled using the second neural network. This neural network takes the current state \(\mathbf{x}\) as the input and produces the corresponding conductance \(\Gm(\mathbf{x})\) as the output. The memristor state \(\mathbf{x}\) is not constrained to a single dimension, enabling a more flexible representation of the device’s internal dynamics.
...
%    \TODO[DONE]{Results for the whole dataset should be provided in the first row.}
    \TODO{It is surprising that for two methods the results for the validation dataset are worse than for the whole dataset. It would be good to show results for the test dataset in another row. Perhaps this would explain this phenomenon (sorry for not suggesting it before).}
  }
...
\begin{center}
    ...
    \midrule
    \(\LossBase\) & \(1.49 \mtimes 10^{-2}\) & \color{ieeegreen} \bfseries \(8.28 \mtimes 10^{-3}\) & \(4.19 \mtimes 10^{-2}\) & \color{ieeered} \bfseries \(5.30 \mtimes 10^{-2}\) \\
    \toprule
    \multicolumn{5}{c}{Whole dataset} \\
    \midrule
    \(\LossBase\) & \(1.52 \mtimes 10^{-2}\) & \color{ieeegreen} \bfseries \(2.23 \mtimes 10^{-3}\) & \(3.58 \mtimes 10^{-2}\) & \color{ieeered} \bfseries \(7.24 \mtimes 10^{-2}\) \\
    \bottomrule
  \end{tabular}
\end{center}
\end{table}
...
The results are presented in Fig.~\ref{fig:hyperparameter_importance}. Both sensitivity analysis methods consistently identified the \textit{MLPG depth} (i.e., the depth of the neural network used to approximate \(\G(\xvec)\)), the \textit{gradient clipping value} \changed{(which sets the maximum allowed gradient norm)} \TODO{explain, and validate !! }, \changed{and \textit{with regularization} (boolean argument which enables or disables additional regularization)} as the dominant hyperparameters \TODO{What about ``With Regularization''}. According to the gain-based measure, their relative importance are 0.07 and 0.56, respectively, whereas the permutation-based analysis yields corresponding values of 0.16 and 0.20. Collectively, these three parameters accounted for approximately 50\% of the observed performance variance. In contrast, the choice of activation function exhibited only a marginal effect (importance score less than \(0.01\)), suggesting that the Neural ODE architecture demonstrates a high degree of robustness with respect to this factor within the examined range.

\begin{figure}[thbp]
  \centering
  \includegraphics[width=\linewidth]{mms_figs/xgboost_permutation_importance.pdf}
  \caption{Hyperparameter importance for the \MemODETwo\  optimization task. Scores are computed using permutation importance applied to an XG Boost surrogate model trained on 200 hyperparameter configurations. Larger values indicate greater relative influence of the corresponding hyperparameter on the model performance, where \changed{MLP-G refers to the neural network used to approximate \(\G(\xvec)\), while MLP-X denotes the neural network used to approximate \(f(\xvec,\ua)\).}
\TODO{Explain abbreviations MLP-G, MLP-X and hyperparameters not mentioned in the text.}
  }
