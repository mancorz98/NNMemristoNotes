...
\begin{equation}
  ...
  \label{eq:loss_con}
\end{equation}
\changed[approved]{where \(\sigma(z) = (1+e^{-z})^{-1}\) is} the sigmoid activation function, \(a\) and \(b\) denote the lower and upper bounds of the admissible parameter range, respectively,
...
The operational characteristics of the constraint function \(\mathcal{L}_{\mathrm{con}}(x)\) \changed[approved]{are} illustrated in Figure~\ref{fig:constrains} for the parameter configuration: \(a=0\), \(b=1\), \(\changed[approved]{f=0.0025}\) over the domain \(x \in (-0.5, 1.5)\).
...
\begin{align}
  ...
  f_{\rm ON}(t)  & = \changed[approved]{\sigma\left(-\beta (\ua(t)-\von)\right)} \label{eq:mms5b},      \\
  f_{\rm OFF}(t) & = 1 - \changed[approved]{\sigma\left(\beta (\ua(t)-\voff)\right)}  \label{eq:mms5c}, 
\end{align}
