...
%ZG removed "physical"
\noindent This study utilizes experimental data obtained from Self-Directed Channel (SDC) memristors doped with tungsten. The structure and properties of these devices have been described in detail in the literature, e.g., in~\changed{\cite{Campbell2017,Garda2024}}.

The experimental setup, illustrated in Fig.~\ref{fig:memristor_setup}, consists of an SDC memristor connected in series with a resistor. The sinusoidal input voltage is supplied by an arbitrary waveform generator. \changed{The} voltage signals are measured using a data acquisition (DAQ) system.
%
Measurements are carried out for the \changed{following combinations of supply voltage amplitudes \(\Vs\) and frequencies \(f\): \(\Vs  \in \{0.5,\ 1.0,\ 1.5\}~\mathrm{[V]}\),} \(f \in \{1,\ 5,\ 10,\ 20,\ 50,\ 100\}~\mathrm{[Hz]}\).
...
\changed{The} analyzed model of a memristor follows the general theoretical framework of memristive devices originally introduced in~\cite{Chua1976}, and is expressed as
...
Let us consider two sequences \((z_{k,t})_{t=1}^T\) \changed{and \((\tilde{z}_{k,t})_{t=1}^T\)}, where \(z \in \{v,i,\dot{v},\dot{i},\ddot{v},\ddot{i}\}\) is a selected variable (voltage or current) or its derivative \changed{obtained from measurements}, \changed{\(\tilde{z}\) is the predicted value of \(z\)}, \(T\) is \changed{the} number of samples and \(k = 1,2,\ldots, N_{\text{traj}}\) is the sequence index.
The \changed{sequences are} normalized utilizing the standard deviation to ensure statistical consistency within each dataset.
...
The \changed{normalized} mean squared error between two sequences \changed{\(({z}_{k,t})_{t=1}^T\) and \((\tilde{z}_{k,t})_{t=1}^T\)} of \(T\) data points is defined as
\begin{equation}
...
The upper bound for the OFF-state resistance~\(R_{\mathrm{OFF}}\) and bounds for the conductance parameter~\(G\) are based on empirical measurements.
...
In the second approach (\MemODETwo), the memristor conductance is modeled using the second neural network. This neural network takes the current state \changed{\(\mathbf{x}\)} as \changed{the} input and produces the corresponding conductance \changed{\(G_m(\mathbf{x})\)} as \changed{the} output. In this approach, the memristor state variable \(\mathbf{x}\) is not constrained to a single dimension, enabling a more flexible representation of the device’s internal dynamics. Moreover, it is not restricted to a physically interpretable range, such as the interval \([0,1]\) in Eq.~\eqref{eq:conductance}.
...
where \(\phi \in [0,1]\) is \changed{the} parameter representing the ratio of the memory dependent current \(\imd\) and total current \(\im\) flowing through the device. The parameters \(\alpha_f\), \(\beta_f\), \(\alpha_r\), and \(\beta_r\) are positive constants characterizing the forward and reverse current behavior along the Schottky barrier.

The inclusion of additional parameters and nonlinearities in the GMMS model increases model's complexity and poses challenges for parameter optimization. \changed{An} additional challenge arises due to the Schottky effect, which introduces nonlinearity into the current–voltage characteristic. When the memristor is connected in series with a resistor, this requires solving a set of differential-algebraic equations (DAEs) of the form
\begin{equation}
  \left\{
  \begin{aligned}
     & \vs(t)    = \vr(t) + \vm(t),                                                      \\
