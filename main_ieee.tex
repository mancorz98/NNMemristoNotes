...
\begin{figure}[htbp]
  ...
  \caption{The \changed{\MemODE}\ approach. The artificial neural network (ANN) evaluates the function \(f\!(x,v)\), which is then integrated by the ODE solver to predict the state at the next time step \(x\!(t + \Delta t)\). The upper dashed feedback loop indicates the recurrent nature of the process, where the output state is fed back as input for subsequent predictions. \changed{The memductance \(\Gm\) is defined as function \(g\) which can be a deterministic function based on the internal variable~\(x\) or the neural network with input \(\mathbf{x}\!(t)\)}. The function \(h\!(\G, v_s\!(t))\) computes the memristor voltage \(\vm\) given the memductance \(\G\) and supply voltage \(v_s\!(t)\).
  }
