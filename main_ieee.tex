...
\section{Conclusions}
\noindent This work presented two neural ODE-based architectures (\MemODEOne\ and \MemODETwo) for data-driven modeling of memristive devices, trained and validated on experimental data from SDC memristors. Both models significantly outperform the MMS and GMMS benchmarks, with \MemODETwo\ achieving the lowest validation loss of \(5.77 \times 10^{-3}\). Training times are comparable to or shorter than those of existing models, though trajectory evaluation is slower due to numerical integration. The proposed framework is applicable to memristors produced in other technologies and provides a foundation for more accurate simulation tools in the design of memristor-based circuits and neuromorphic systems.
