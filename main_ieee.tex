...
\begin{equation}
  \Loss_{\rm con}(x) = \mathbb{E}\left[ {\sigma\left(-\tfrac{x-a}{\changed{\gamma}}\right)}(a-x)^{2}+{\sigma\left(\tfrac{x-b}{\changed{\gamma}}\right)}(x-b)^{2}\right],
  \label{eq:loss_con}
\end{equation}
...
The term \(\sigma(-(x-a)/\changed{\gamma})(a-x)\) penalizes values of \(x\) below the lower bound \(a\),
while the term \(\sigma((x-b)/\changed{\gamma})(x-b)\) penalizes values of \(x\) above the upper bound \(b\).
Squaring ensures that the penalty is non-negative and grows  with the violation magnitude.

The operational characteristics of the constraint function \(\Loss_{\rm con}(x)\) are illustrated in Figure~\ref{fig:constrains} for the parameter configuration: \(a=0\), \(b=1\), \(\changed{\gamma}=0.0025\) over the domain \(x \in (-0.5, 1.5)\).
...
\begin{figure}[!t]
  ...
  %ZG modified caption
  \caption{The constraint function \(\Loss_{\rm con}(x)\) for \(a=0\), \(b=1\), \(\changed{\gamma}=0.0025\).}
...
\begin{align}
  \begin{split}
    \im(t)     & = \changed{\eta} \imd(v, x) + (1-\changed{\eta}) i_{\rm d}(v),                \\
