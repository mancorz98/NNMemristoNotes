...
\end{align}
The terms \(\LossBase\), \(\LossVel\), and \(\LossCur\) represent the NMSE (\textit{Normalized Mean Squared Error}) of signal values, first derivatives, and second derivatives, respectively, between predicted and measured trajectories. The \changed{terms} \(\Loss_{\rm con}(\changed{x_i})\) enforces physical constraints. The weighting coefficients are computed at each step using the Geometric Loss Strategy (GLS)~\cite{Chen2018}.
...
\begin{figure}[htbp]
  ...
  \end{adjustbox}
  \caption{The \changed[approved]{\MemODE}\ approach. The artificial neural network (ANN) evaluates the function \(f\!(x,v)\), which is then integrated by the ODE solver to predict the state at the next time step \(x\!(t + \Delta t)\). The upper dashed feedback loop indicates the recurrent nature of the process, where the output state is fed back as input for subsequent predictions. \changed[approved]{The memductance \(\Gm\) is defined as \changed{the} function \(g\) which \changed{is} a deterministic function based on the internal variable~\(x\) (\removed{as in }\MemODEOne) or the neural network with \changed{the} input \(\mathbf{x}\!(t)\) (\removed{as in }\MemODETwo)}. The function \(h\!(\G, v_s\!(t))\) computes the memristor voltage \(\vm\) given the memductance \(\G\) and supply voltage \(v_s\!(t)\).
  }
