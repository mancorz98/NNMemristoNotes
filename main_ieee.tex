...
Among the widely adopted models for practical memristive devices is the \textit{Stanford} model~\cite{sheridan2011device, guan2012spice, jiang2014verilog, chen2015compact, di2022oscillatory}, which provides a physics-based compact representation of bipolar resistive random access memory (RRAM) structures incorporating metal-oxide layers. As an extended memristor formulation, this framework successfully describes the nonlinear resistive switching dynamics observed across a diverse range of device implementations, particularly capturing critical phenomena such as apparent threshold behavior, temporal voltage-dependent switching characteristics, and multilevel conductance programming capabilities.

The \textit{Generalized Metastable Switch} (GMS) model~\cite{Molter2016} provides enhanced accuracy in modeling Self-Directed Channel (SDC) memristors through a semi-empirical formulation where the state variable dynamics depend on transition probabilities between metastable states. However, such symbolic models exhibit limited generalization capability when driving signal parameters vary, posing challenges for applications with highly dynamic operating conditions such as memristor-based chaotic oscillators.

Memristive circuits exhibit complex dynamical phenomena, including amplitude control mechanisms. In~\cite{Galias2018}, a memristive band pass filter circuit was analyzed, revealing both partial amplitude control (PAC), enabling independent rescaling of selected state variables, and total amplitude control (TAC), facilitating uniform scaling of all system variables through modification of a single parameter. These properties are particularly advantageous in applications where signal amplitude adjustment is required without employing additional amplification stages.
To address these limitations, Minati et al.~\cite{Minati2020} introduced the \textit{Mean Metastable Switch} (MMS) model, which simplifies the GMS framework by computing the derivative of the internal state variable through linear interpolation of mean values obtained from Gaussian sampling of each probabilistic function, while eliminating the Schottky effect to preserve model simplicity. Subsequently, Laguna-Sánchez et al.~\cite{LagunaSanchez2023} proposed the \textit{Generalized Mean Metastable Switch} (GMMS) model, which extends the MMS framework by reincorporating the Schottky diode effect originally described in~\cite{Molter2016}.
}
\changed{
Numerical simulation of memristors with bounded states poses computational challenges, as window functions constraining internal variables introduce discontinuities that cause artifacts in standard integration methods. Paper~\cite{Galias2019} addressed this through a Poincaré map approach, partitioning the state space into smooth regions to enable accurate trajectory computation across discontinuity boundaries.
}
...
\noindent \changed{The neural network training is implemented using the \texttt{JAX} library~\cite{jax2018github} for array-oriented numerical computation and \texttt{Equinox}~\cite{kidger2021equinox} for neural network construction in \texttt{Python}. The \texttt{Diffrax} library~\cite{kidger2021on} provides differentiable ODE solvers enabling efficient gradient computation through adjoint sensitivity methods~\cite{wang2009minimal, stumm2010new, Ricky2018, Ricky2021}. The \texttt{tsit5} adaptive Runge-Kutta solver~\cite{Tsitouras2011} is employed for its computational efficiency and numerical accuracy, with adaptive step-size control ensuring stable integration during training.
}
