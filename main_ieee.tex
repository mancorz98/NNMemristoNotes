...
\begin{document}
\title{Neural ODE based modeling of memristive \changed{devices}}
...
\comment{Herce powinny być raczej w normalnym foncie roman a nie matematycznym, szczególnie ``z'' w wersji mathrm wygląda dziwnie.}
...
\begin{figure}[!t]
  ...
  \caption{Schematic diagram of the measurement setup used for the SDC memristor. The device is connected in series with a resistor, and the input voltage is applied via an arbitrary waveform generator. Voltage measurements are acquired using a data acquisition (DAQ) system.
    ...
    \TODO[DONE]{\(f_s\) should be replaced by \(f\) }
  }
...
Let us consider two sequences \((z_{k,\sind})_{\sind=1}^{\snum}\) and \((\tilde{z}_{k,\sind})_{\snum=1}^{\snum}\), where \(z \in \{v,i,\dot{v},\dot{i},\ddot{v},\ddot{i}\}\) is a selected variable (voltage or current) or its derivative obtained from measurements, \(\tilde{z}\) is the predicted value of \(z\), \(\snum\) is the number of samples and \(k = 1,2,\ldots, N_{\text{traj}}\) is the sequence index.
...
The normalized mean squared error between two sequences \changed{\(({z}_{k,\sind})_{\sind=1}^{\snum}\) and \((\tilde{z}_{k,\sind})_{\sind=1}^{\snum}\) of \(\snum\)} data points is defined as
\begin{equation}
...
To enforce parameter bounds within specified intervals, a sophisticated constraint loss function utilizing smooth sigmoid transitions is implemented. This component prevents parameter drift beyond physically meaningful ranges while maintaining differentiability for gradient-based optimization algorithms. The constraint loss for the admissibility interval \(x\in[a,b]\) is mathematically expressed as:
%ZG \sigma used in the next equation, merged two lines into one
\begin{equation}
  \Loss_{\rm con}(x) = \mathbb{E}\left[ {\sigma\left(-\tfrac{x-a}{f}\right)}(a-x)^{2}+{\sigma\left(\tfrac{x-b}{f}\right)}(x-b)^{2}\right],
  \label{eq:loss_con}
\end{equation}
where \(\sigma(z) = (1+e^{-z})^{-1} \) is the sigmoid activation function, \(a\) and \(b\) denote the lower and upper bounds of the admissible parameter range,
...
The operational characteristics of the constraint function \(\Loss_{\rm con}(x)\) are illustrated in Figure~\ref{fig:constrains} for the parameter configuration: \(a=0\), \(b=1\), \(f=0.0025\) over the domain \(x \in (-0.5, 1.5)\).
...
\begin{figure}[!t]
  ...
  %ZG modified caption
  \caption{The constraint function \(\Loss_{\rm con}(x)\) for \(a=0\), \(b=1\), \(f=0.0025\).}
  \label{fig:constrains}
\end{figure}

\subsection{Neural ODE based Memristor Modeling}

\noindent In this study, two neural network-based modeling approaches are proposed. The first one, denoted as \textbf{\MemODEOne}, employs a deterministic formulation of the memristor \changed{memductance} \(\G\), while the function \(f\) defining the dynamics of internal variables is modeled using a neural network (see Fig.~\ref{fig:nn_structure}). In the second one, referred to as \textbf{\MemODETwo}, both \(\G\) and \(f\) are modeled using neural networks (see Fig.~\ref{fig:nn_structure_dual}).
...
\begin{figure}[htbp]
  ...
  \caption{The \MemODETwo\ approach. The the artificial neural network (ANN) evaluates the function \(f(\mathbf{x},v)\), which is then integrated by the ODE solver to predict the state at the next time step \(\mathbf{x}(t\!+\!\Delta t)\). The second ANN models the memristor \changed{memductance} \(\Gm(\mathbf{x})\) based on the next state \(\mathbf{x}(t\!+\!\Delta t)\). \changed{
   \changed{
    The function \(h(\G, v_s(t))\) computes the memristor voltage \(\vm\) given the \changed{memductance} \(\G\) and supply voltage \(v_s(t)\).
  }
    }
  \TODO[DONE]{Replace \(\G_m\) by \(\G\).} \TODO[DONE]{Explain what is \(h(\G)\).} 
  }
...
In the \MemODEOne\ approach, the memristor \changed{memductance} is defined on the basis of the physical mechanisms underlying self-directed channel (SDC) devices. In such memristors, the resistance evolves as a result of \(\mathrm{Ag^+}\) ions migration and the formation of conductive filaments. Consequently, the device \changed{memductance} exhibits a continuous transition between different resistance states. This transition can be described as a weighted combination of the limiting resistance values, modulated by the internal variable \(x\)
\begin{equation}
  \Gm(x) = \frac{x}{R_{\rm ON}} + \frac{1 - x}{R_{\rm OFF}},
  \label{eq:conductance}
\end{equation}
where \(\Ron\) and \(\Roff\) denote the resistances in the low-\changed{memductance} and high-\changed{memductance} states, respectively. During the optimization process, the network parameters (weights \(w_{k,i}\) and biases \(b_k\)) are tuned jointly with the values of \(R_{\rm ON}\) and \(R_{\rm OFF}\). For this approach the following constraints are applied:
\begin{itemize}
\item \(R_{\mathrm{ON}} \in (0, R_{\mathrm{OFF}})\kOhm\),
\item \(R_{\mathrm{OFF}} \in (R_{\mathrm{ON}}, 200)\kOhm\),
\item \(x(t) \in (0, 1)\).
\end{itemize}
The upper bound for the OFF-state resistance~\(R_{\mathrm{OFF}}\) is based on empirical measurements. Remaining constraints are consequences of physically meaningful values of parameters and variables. 

In the \MemODETwo approach, the memristor \changed{memductance} is modeled using the second neural network. This neural network takes the current state \(\mathbf{x}\) as the input and produces the corresponding \changed{memductance} \(\Gm(\mathbf{x})\) as the output. The memristor state \(\mathbf{x}\) is not constrained to a single dimension, enabling a more flexible representation of the device’s internal dynamics.
...
\begin{itemize}
\item \(\Gm(\mathbf{x}) \in (0, 2) \mS\).
\end{itemize}
The upper bound is based on measurements. 
\comment{Since constraints on \(R_{\mathrm{ON}}\), \(R_{\mathrm{OFF}}\), and \(\G(\mathbf{x})\) are based on empirical measurements there should be a relation between these constraints. This is not clear from the values used.}
...
The MMS model is a simplified representation of memristor's behavior, focusing on the average switching characteristics rather than the detailed dynamics. It captures essential features of the device by describing transitions between
%\TODO[Done]{``a two-state system'', this is not a two state system}
the high-resistance state (HRS) and the low-resistance state (LRS) based on the applied voltage. This model describes memristor's dynamics as
...
\end{align}
where \(\sigma(z) = (1+e^{-z})^{-1}\), \(\beta= \frac{q}{kT}\), \(k\) is the Boltzmann constant, \(T\) is the absolute temperature, \(q\) represents the elementary charge, while \(\von\) and \(\voff\) denote the switching voltages to LRS and HRS states, respectively. The instantaneous \changed{memductance} of the memristor is defined in~\eqref{eq:conductance}.
...
\begin{align}
  \begin{split}
    \im(t)     & = \phi \imd(v, x) + (1-\phi) i_{\rm d}(v),                \\
    i_d(v)     & = \alpha_{\rm f} e^{\beta_{\rm f} v} - \alpha_{\rm r} e^{-\beta_{\rm r} v}, \\
    \imd(v, x) & = \Gm(x) v,
  \end{split}
  \label{eq:gmms}
\end{align}
...
During the hyperparameter optimization stage, the optimal configuration of the \MemODEOne\ architecture found comprises four hidden layers with 64, 128, 128, 64 neurons in consecutive layers. The model was trained using the \texttt{AdamW} optimizer with a learning rate of approximately \(1.89 \times 10^{-2}\) and a batch size of 2. The swish activation function \(f(x)=x\cdot\sigma(x)\) was employed for the hidden layers, while the final layer utilized a \(\mathrm{linear}\) activation function \(f(x)=x\).  \comment{Please, check the formulas for activation functions.}

For the \MemODETwo\ architecture, the optimal neural network configuration responsible for computing \(f(\xvec,\ua)\) consisted of four hidden layers with 120, 240, 240, 120 neurons in consecutive layers and with 6 latent states,\changed{ corresponding to the dimensionality of the internal state vector \(\mathbf{x}\)} \TODO[DOING]{explain what is the latent state}. The auxiliary neural network responsible for computing \(\G\) included one hidden layer, containing 80 neurons. Both networks were trained using the \texttt{AdaBelief} optimizer with a learning rate of approximately \(6.30 \times 10^{-3}\) and a batch size of 14. The first neural network employed the \(\mathrm{GELU}\) activation function \changed{\( f(x) = x \cdot \Phi(x)   \approx 0.5 x (1 + \tanh[\sqrt{2/\pi} (x + 0.044715 x^3)]) \)} in the hidden layers and a \(\mathrm{linear}\) activation in the output layer, whereas the second network used the \(\mathrm{relu}\) activation function \(f(x)=\max{}(0,x)\) with the \(\mathrm{sigmoid}\) output activation.\TODO[DONE]{We should add a definition of GELU or give a reference.}
%

The median training duration for the \MemODEOne\ architecture is approximately 11 minutes 21 seconds, while the \MemODETwo\ architecture requires approximately 13 minutes 51 seconds when utilizing a single CPU (Intel i9-9900X) \TODO{How many cores are used?} For comparative analysis, the median computational time necessary for parameter optimization of the MMS model is approximately 20.4 minutes, whereas the GMMS model requires approximately 8.4 minutes using the same hardware. The neural network architectures were trained employing a standard deep learning framework based on gradient descent optimization algorithms. Conversely, the parameter identification methodology for the analytical models (MMS and GMMS) necessitated a two-stage optimization protocol. Initially, a computationally intensive global search algorithm was executed over the whole parameter space to obtain an approximate yet robust initial parameter estimate. Subsequently, a local refinement procedure employing gradient-based optimization was applied. This methodology reflects the inherent computational complexity of determining appropriate initial parameters for analytical models and accounts for their extended calibration duration relative to the single-iteration training cost of neural network-based architectures.

The computation time for trajectory generation comprising 18 trajectories using the MMS model is approximately 0.047 seconds, whereas the GMMS model requires approximately 0.051 seconds. In contrast, the \MemODETwo\ architecture needs approximately 3.97 seconds, while the \MemODEOne\ architecture requires approximately 5.56 seconds to generate an equivalent trajectory ensemble. The neural network models were integrated numerically using a relative error tolerance of \(10^{-3}\) and an absolute error tolerance of \(10^{-6}\).

% \TODO{
%   0.0474 seconds for all 18 trajectories for MMS model,  0.0510  seconds for GMMS model, 3.9664 seconds for \MemODETwo{} 5.5620 seconds       for \MemODEOne{}
% }
...
%    \TODO[DONE]{Use black dahsed lines instead of black solid lines in the legends.}
    \TODO[DONE]{Replace \(v_m\) by \(v\) and \(i_m\) by \(i\).}
  }
...
\begin{table}
  \caption{Comparative analysis of the loss function \(\LossBase\). Values are reported for different supply voltage amplitudes \(\Vs\) and frequencies \(f_s\). \changed{Row-wise minimum and maximum loss values are color-coded in green and red, respectively. Records belonging to the validation dataset are demarcated by bounding boxes.} }
  \label{tab:mms_comp}
  \centering
...
Table~\ref{tab:mms_comp_test} reports the average loss function values \(\LossBase\) calculated on the validation dataset and the whole dataset for the proposed and existing models, providing a quantitative evaluation of their generalization performance across unseen memristor dynamics.
The best result of \(5.77 \times 10^{-3}\) is obtained for the \MemODETwo\ model. The above two times reduction of the average values of the loss function for both proposed method when compared to existing methods demonstrates the enhanced modeling capability achieved through the neural ODE framework.

\begin{table}
  \caption{Comparative analysis of the loss function \(\LossBase\) for the validation dataset and the whole dataset.
%    \TODO[DONE]{Results for the whole dataset should be provided in the first row.}
    \TODO[DONE]{It is surprising that for two methods the results for the validation dataset are worse than for the whole dataset. It would be good to show results for the test dataset in another row. Perhaps this would explain this phenomenon (sorry for not suggesting it before).}
  }
...
To quantify the relative influence of individual hyperparameters on the model performance, a surrogate-based importance analysis is performed. An XGBoost gradient boosting model~\cite{Chen2016} is trained on the explored configurations to predict validation loss. The surrogate \changed{model} achieved an \(\mathrm{R}^2\) value of 0.99, indicating high fidelity in capturing the dependence of model performance on hyperparameter selection\changed{, where \(\mathrm{R}^2 = 1 - \frac{\sum_i (y_i - \hat{y}_i)^2}{\sum_i (y_i - \bar{y})^2}\) represents the coefficient of determination measuring the proportion of explained variance; here \(y_i\) are the observed values, \(\hat{y}_i\) are the model predictions, and \(\bar{y}\) is the mean of observed values}. Importance is first estimated using the \emph{gain} metric, which reflects the cumulative improvement in predictive accuracy attributed to each hyperparameter across the ensemble. Normalization ensured that scores sum up to unity, allowing for direct interpretation as relative contributions to performance variability.
...
The results of the sensitivity analysis are presented in Fig.~\ref{fig:hyperparameter_importance}. Both evaluation methods consistently identified four parameters as dominant hyperparameters influencing model performance: the \textit{number of epochs}, the \textit{gradient clipping value} (which defines the maximum allowable gradient norm), the \textit{learning rate}, and the \textit{ANN-\(\mathcal{G}\) depth} (the depth of the neural network used to approximate \(\mathcal{G}(\mathbf{x})\)).

The relative importance scores for these four parameters (listed in the order above) are detailed below, comparing the results of the gain-based and permutation-based analyses:
...
Collectively, the three most significant parameters accounted for approximately 70\% of the total observed performance variance (derived from summing the three highest scores in the permutation analysis). In contrast, the choice of activation function exhibited a negligible effect (importance score \(< 0.01\)), indicating that the Neural ODE architecture demonstrates a high degree of robustness with respect to this factor across the examined range.
}
\begin{figure}[thbp]
  \centering
  \includegraphics[width=\linewidth]{mms_figs/xgboost_permutation_importance.pdf}
  \caption{Hyperparameter importance for the \MemODETwo\  optimization task. Scores are computed using permutation importance applied to an XG Boost surrogate model trained on 200 hyperparameter configurations. Larger values indicate greater relative influence of the corresponding hyperparameter on the model performance, where \changed{MLP-\(\G\) refers to the neural network used to approximate \(\G(\xvec)\), while MLP-\(f\) denotes the neural network used to approximate \(f(\xvec,\ua)\).}
\TODO[DONE]{Explain abbreviations MLP-G, MLP-X and hyperparameters not mentioned in the text.}
  }
  \label{fig:hyperparameter_importance}
\end{figure}

The mean values of the validation loss obtained during the hyperparameter optimization procedure for the categorical hyperparameters are presented in Fig.~\ref{fig:categorical_hyperparameters}. For each categorical option, the reported mean is computed by marginalizing over all remaining hyperparameters, while the corresponding \(95\%\) confidence intervals are indicated by the error bars. The figure summarizes the influence of (a) the number of latent states Fig.~\ref{1a3}, (b) the optimizer type Fig.~\ref{1b3}, as well as the activation functions employed in the two neural networks constituting the \MemODETwo\ architecture Fig.~\ref{1c3}–\ref{1d3}. This analysis highlights the relative sensitivity of the model performance to discrete architectural and training-strategy choices.
...
\begin{figure}[!t]
  ...
  \caption{Validation loss obtained during hyperparameter optimization for the categorical hyperparameters: (a) number of latent states, (b) optimizer type for the first neural network, (c) activation function for the first neural network, used to approximate \(f(\xvec,\ua)\), and (d) activation function for the second neural network, used to approximate \(\G(\xvec)\). Mean values are computed by averaging over all other hyperparameters, with error bars indicating the \(95\%\) confidence interval.}
...
The proposed models were applied to modeling of real Self-Directed Channel (SDC) memristors. 
...
The findings of this study underscore the potential of neural ODEs as a versatile and effective framework for modeling complex physical systems, with implications for the design and analysis of memristor-based circuits and applications.
The methods developed herein pave the way for more accurate and efficient simulation tools, facilitating advancements in memristor technology and its integration into emerging computing paradigms, and open avenues for future research in data-driven modeling of nonlinear dynamical systems.
