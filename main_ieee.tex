...
\section{\changed{Modeling Memristors using Neural Networks}}

%ZG removed "physical"
\noindent This study utilizes experimental data obtained \changed{from Self-Directed Channel} (SDC) memristors doped with tungsten. The structure and properties of these devices have been described in detail in the literature, e.g., in~\cite{Garda2024, Campbell2017}.

The experimental setup, illustrated in Fig.~\ref{fig:memristor_setup}, \changed{consists} of an SDC memristor connected in series with a resistor. The sinusoidal input voltage \changed{is} supplied by an arbitrary waveform generator, while the voltage signals \changed{are} measured using a data acquisition (DAQ) system.

Measurements \changed{are} carried out for various combinations of supply voltage amplitudes and frequencies. Specifically, the amplitude of the applied voltage was varied as $\Vs  \in \{0.5,\ 1.0,\ 1.5\}~\mathrm{[V]}$, and the frequency was selected from the set \(f \in \{1,\ 5,\ 10,\ 20,\ 50,\ 100\}~\mathrm{[Hz]}\).
...
\noindent The analyzed model \changed{of a memristor follows} the general theoretical framework of memristive devices originally introduced in~\cite{Chua1976}, and is expressed \changed{as}
...
\end{align}
\end{subequations}
where \(\xvec\) denotes the vector of internal state variables, whose temporal evolution is governed by the \changed{function} \(f\), and \(\G\) represents the memductance of the device. \changed{The} fundamental challenge in memristor modeling lies in accurately identifying the functional forms of \(\G(\xvec,\ua)\) and \(f(\xvec,\ua)\).

\subsection{\changed{Neural ODE based Memristor Modeling}}

\TODO{I suggest to change the acronym for the first method to ``SingleNN-MemODE''. What is your opinion?}

\noindent In this study, two neural network-based modeling approaches are \changed{proposed}. The first \changed{one}, denoted as \textbf{Det-MemODE}, employs a deterministic formulation of the memristor conductance \changed{$\G$, while the function $f$ defining the dynamics of internal variables is modeled using a neural network. In} the second \changed{one}, referred to as \textbf{Dual-NN-MemODE}, \changed{both \changed{$\G$ and $f$ are modeled} using neural networks}.
...
\changed{In the second approach (Dual-NN-MemODE), the memristor conductance is modeled using the second neural network.} \TODO{More description needed.}

\changed{C}onceptual architecture\changed{s} \changed{are} illustrated in Fig.~\ref{fig:nn_structure} for the Det-MemODE and in Fig.~\ref{fig:nn_structure_dual} for the Dual-NN-MemODE.
...
\begin{figure}[t!]
  ...
  \caption{ Conceptual diagram illustrating the use of a neural network to simulate the dynamics of the memristor, for the Det-MemODE, with schematic representation of the Neural ODE framework, where \changed{the} artificial neural network (ANN) evaluates \changed{the function \(f(x,v)\)}, which is then integrated by \changed{the} ODE solver to predict the state at the next time step \(x(t + \Delta t)\). The dashed feedback loop indicates the recurrent nature of the process, where the output state is fed back as input for subsequent predictions.\TODO{Replace $f_x$ by $f$ in the figure.}}
...
\begin{figure}[t!]
  ...
  \caption{ Conceptual diagram illustrating the use of a neural network to simulate the dynamics of the memristor, for the Dual-NN-MemODE, with schematic representation of the Neural ODE framework, where \changed{the} artificial neural network (ANN) evaluates \changed{the function \(f(x,v)\)}, which is then integrated by \changed{the} ODE solver to predict the state at the next time step \(x(t + \Delta t)\).
    ...
    \TODO{Replace $f_x$ by $f$ in the figure.}
    \TODO{Description needed for the second ANN.}
  }
...
\noindent The neural network \changed{training is} implemented using the \texttt{PyTorch} deep learning framework~\cite{paszke2019pytorchimperativestylehighperformance}, within the \texttt{Python} programming environment, \changed{utilizing} its automatic differentiation capabilities. To enable seamless integration of ordinary differential equations (ODEs) within the neural network training paradigm, the specialized \texttt{torchdiffeq} library \changed{is} employed. This library provides differentiable ODE solvers that facilitate efficient gradient computation with respect to solution trajectories through the adjoint sensitivity method~\cite{Ricky2018, Ricky2021, torchdiffeq}. This approach enables end-to-end training of neural differential equation models by maintaining gradient flow through the numerical integration process, which is essential for learning \changed{dynamics of systems governed by ODEs}.

%ZG replaced "embedded error estimation" by "error estimation"
Specifically, the \texttt{dopri5} (Dormand-Prince 5th order) adaptive Runge-Kutta solver \changed{is} utilized for its superior balance between computational efficiency and numerical accuracy. This solver \changed{employs error estimation} for adaptive step-size control, ensuring stable integration of the memristor dynamics while maintaining computational tractability during training.

The neural network architecture, illustrated in Figure~\ref{fig:nn-arch}, employs an expansion–compression (diamond-shaped) topology that incorporates a \changed{double} reduction in dimensionality in both the initial and final hidden layers.
\changed{This architectural configuration substantially decreases the overall number of trainable parameters and helps to mitigate vanishing and exploding gradient issues commonly encountered in deep architectures processing temporal sequences.}
...
\begin{figure}[ht!]
  ...
  \caption{Architecture of the feedforward neural network used within the Neural ODE framework for memristor dynamics modeling. The network consists of an input layer receiving two inputs, multiple hidden layers with decreasing-increasing-decreasing neuron counts \changed{$\left(\lfloor N/2 \rfloor, N,\ldots,N, \lfloor N/2 \rfloor\right)$}, activation functions between layers, and a single output. The Neural ODE block encompasses the core computational layers that approximate the function \(f(\xw(t), \ua(t))\) \changed{of the ODE system}.}
...
\changed{Several techniques are tested for the optimization of network weights and the \texttt{Adam} (Adaptive Moment Estimation) optimizer is selected due to its superior performance.}
%ZG removed ", as described in"
The choice of \changed{an} optimizer and its parameters \changed{is} guided by hyperparameter tuning conducted with the \texttt{Optuna} framework~\cite{Akiba2019}. The hyperparameter search space \changed{comprises} the learning rate, weight decay, and \changed{the} batch size, or architecture parameters like \changed{the} width or \changed{the} depth of \changed{the} neural network.
%removed ", while"
The optimal configuration was selected based on validation loss minimization. \TODO{What is ``validation loss minimization''?}
...
%  \end{split}
\end{align}
\changed{The first term \(\mathcal{L}_{\mathrm{base}}\) denotes the primary error computed as} the mean squared error (MSE) \changed{between} predicted and reference values. \(\mathcal{L}_{v}\) enforces accurate modeling of \changed{derivatives of state variables}, \(\mathcal{L}_{c}\) promotes consistency of the trajectories in terms of their curvature by \changed{computing MSE of second derivatives}, and \(\mathcal{L}_{\mathrm{con}}(x)\) \changed{enforces fulfillment of} physical or structural constraints that the model is required to satisfy, including
...
\changed{Their} values \changed{are} calculated during each optimization step using the Geometric Loss Strategy \changed{(GLS)~\cite{Cipolla2018, Chen2018}} \changed{using} the \changed{Algorithm~\ref{alg:adaptive_loss_balancing}}.
...
\changed{Let us consider the sequence $(z_{k,t})_{t=1}^T$, where $z \in \{v,i,\dot{v},\dot{i},\ddot{v},\ddot{i}\}$ is a selected variable (voltage or current) or its derivative, $T$ is total number of samples and $k = 1,2,\ldots, N_{\text{traj}}$ is the sequence index.}
...
\changed{The sequence is normalized utilizing the standard deviation to ensure statistical consistency within each dataset}.
The trajectory-specific standard deviation is calculated using the \changed{unbiased estimator}
\begin{equation}
...
\changed{This} standardization approach enhances the numerical stability and convergence properties of optimization algorithms employed in neural network training procedures.

By implementing trajectory-specific normalization rather than global standardization, the methodology preserves the inherent temporal dynamics and statistical properties unique to each circuit configuration while simultaneously ensuring that amplitude disparities do not introduce systematic bias \changed{during predictive modeling procedures}. 
%during pattern recognition, feature extraction, or predictive modeling procedures.
This approach is particularly advantageous in scenarios involving heterogeneous circuit topologies or varying operational conditions, where maintaining the relative temporal \changed{structure is} paramount for accurate system identification and dynamic analysis.
...
The mean squared error \changed{between two sequences $(\hat{y}_i)_{i=1}^n$ and $(y_i)_{i=1}^n$} of $n$ data points is defined as
...
The first-order dynamics comparison \changed{$\mathcal{L}_{v}$} enables evaluation of temporal derivative agreement between model predictions and experimental data, particularly critical for capturing rapid memristor state transitions
...
\begin{equation}
  \begin{split}
    ...
    \changed{\sigma\left(-\tfrac{x-a}{f}\right)}(a-x)+\changed{\sigma\left(\tfrac{x-b}{f}\right)}(x-b)
...
% \begin{itemize}
\changed{where $\sigma(z) = (1+e^{-z})^{-1}$ is} the sigmoid activation function,
%ZG removed next line
%(implicitly used),
\(a\) and \(b\) denote the lower and upper bounds of the admissible parameter range, respectively,
\(f\) is a \changed{scaling} factor controlling the steepness of the transition near the bounds, and
\(\mathbb{E}[\cdot]\) denotes the \changed{expected value} (ensemble average over samples or trajectories).
\changed{The} term \changed{$\sigma(-(x-a)/f)(a-x)$} penalizes values of \(x\) below the lower bound \(a\),
\changed{while} the term \changed{$\sigma((x-b)/f)(x-b)$} penalizes values of \(x\) above the upper bound \(b\). 
\changed{Squaring} ensures \changed{that} the penalty is non-negative and grows  with the violation magnitude.

The operational characteristics of the constraint function \(\mathcal{L}_{\mathrm{con}}(\changed{x})\) \changed{is} illustrated in Figure~\ref{fig:constrains} for the parameter configuration: \(a=0\), \(b=1\), $\changed{f=0.0025}$ \TODO{(verify $f$)} over the domain \(\changed{x} \in (-0.5, 1.5)\).
%ZG next line removed to save space
...
%ZG removed "feasible"
The function \changed{$\mathcal{L}_{\mathrm{con}}(x)$} exhibits minimal penalty in the region \(x\in(0,1)\) and progressively increasing penalties as the \changed{parameter $x$ exceeds} the boundary limits. 
...
\begin{figure}[!t]
  ...
  %ZG modified caption
  \caption{\changed{The} constraint function \(\mathcal{L}_{\mathrm{con}}(\changed{x})\) for \changed{$a=0$, $b=1$, $f=0.0025$}.}
...
%ZG removed "critical"
The following constraint intervals \changed{are} applied to
%critical
memristor model parameters for the Det-MemODE and Dual-NN-MemODE approaches:
...
\item \(x(t) \in (0, 1)\),
  %– This temporal constraint maintains the internal state variable within the normalized domain throughout the entire simulation duration, preserving the physical interpretation of \(x(t)\) as a fractional parameter representing the extent of ionic migration or structural modification within the memristive element.
\item \(\G \in (0, 2) \rm mS\).
  %– This constraint limits the conductance parameter to a realistic range based on empirical measurements of memristor devices, preventing unphysically high conductance values that could lead to numerical instability or divergence during training, and ensuring passive device nature.
\end{itemize}
\TODO{What is $x(t)$ for the Dual-NN-MemODE?}\\
\changed{The upper bound for the OFF-state resistance~$R_{\mathrm{OFF}}$ and bounds for the conductance parameter~$G$ are based on empirical measurements.
  Remaining constraints are consequences of physically meaningful values of parameters and variables.}\\
\TODO{Is the objective function common for all models? If so, perhaps section ``C. Objective Function'' could be placed before section ``A. Neural ODE based Memristor Modeling''. }
...
\subsection{\changed{Existing} Memristor \changed{Models} for Comparison}

%ZG removed "introduced in", removed " described in" 
\noindent To evaluate the effectiveness of the proposed \changed{models}, \changed{their performance is} compared with the \changed{performance} of two \changed{existing memristor} models: the Mean Metastable Switch (MMS) model~\cite{Minati2020} and the  Generalized Mean Metastable Switch Memristor (GMMS) model~\cite{Molter2016, Ostrovskii2021}.

%\subsubsection{Mean Metastable Switch (MMS) Model}

\changed{The MMS model} is a simplified representation of \changed{memristor's} behavior, focusing on the average switching characteristics rather than the detailed dynamics. It captures \changed{essential features} by modeling the device as a two-state system, where the resistance can switch between a high-resistance state (HRS) and a low-resistance state (LRS) based on the applied voltage. This model describes the memristor dynamics \changed{as}
%ZG replaced "on" by "ON" and "off by ""OFF
%ZG merged three equations into one align environment
\begin{align}
  \label{eq:mms}
  \dert{x} & = \frac{1}{\tau} \left( f_{\changed{\rm ON}}(t) (1-x) - f_{\changed{\rm OFF}}(t) x \right), \\%ZG added comma, replaced "\big[" by "\left(" and "\big]" by $\right)$
%\end{align}
%where
%\begin{align}
  f_{\rm ON}(t)  & = \changed{\sigma\left(-\beta (\ua(t)-\von)\right)} \label{eq:mms5b}, \\%ZG added comma, replaced "\frac{1}{1+e^{-\beta (\ua(t)-\von)}}" by \sigma(-\beta (\ua(t)-\von))
  f_{\rm OFF}(t) & = 1 - \changed{\sigma\left(\beta (\ua(t)-\voff)\right)}  \label{eq:mms5c}, %ZG added comma, replaced "\frac{1}{1+e^{-\beta (\ua(t)-\voff)}}" by 
\end{align}
%ZG removed empty line
...
where \changed{$\sigma(z) = (1+e^{-z})^{-1}$,} \(\beta= \frac{q}{kT}\), \(k\) is the Boltzmann constant, \(T\) is the absolute temperature, \(q\) represents the elementary charge, \changed{while $\von$ and $\voff$ denote the switching voltages to LRS and HRS states, respectively}. The instantaneous conductance of the memristor is defined \changed{in~\eqref{eq:conductance}}.
...
\changed{In the GMMS model the memristor is represented as a parallel connection of \changed{memory-dependent element} and a Schottky diode~\cite{Molter2016, Ostrovskii2021}}. This formulation extends the original metastable switch concept by incorporating nonlinear current--voltage characteristics, enabling a more accurate representation of \changed{memristor's} behavior across a wide range of operating conditions.
\changed{The} evolution of the state variable is governed \changed{by~\eqref{eq:mms}}, while the current--voltage relationship is
\begin{align}
  \begin{split}
    \im(t)      & = \phi \imd(v, \changed{x}) + (1-\phi) i_d(v), \\ %ZG added comma
    i_d(v)    & = \alpha_f e^{\beta_f v} - \alpha_r e^{-\beta_r v}, \\ %ZG added comma
    \imd(v, \changed{x}) & = \G_m(x) v,                           %ZG added comma
  \end{split}
  \label{eq:gmms}
\end{align}
%ZG replaced "current through the device" by "current of the device"
where \(\phi \in [0,1]\) is a parameter representing the \changed{ratio of the memory dependent current $\imd$ and total current $\im$ flowing through the device}. The parameters \(\alpha_f\), \(\beta_f\), \(\alpha_r\), and \(\beta_r\) are positive constants characterizing the forward and reverse current behavior along the Schottky barrier.
\TODO{Change of notations. $\imd$ - memory dependent current, previously $\im$, $\im$ - the total current, previously $i$. Please confirm whether this is ok.}

%ZG removed Eq.~\eqref{eq:nonlinear_eq}
\changed{The} inclusion of additional parameters and nonlinearities \changed{in the GMMS model} increases model's complexity and poses challenges for parameter optimization. \changed{An} additional challenge arises due to the Schottky effect, which introduces nonlinearity into the current–voltage characteristic. When the memristor is connected in series with a resistor, this requires solving \changed{a set of differential-algebraic equations (DAEs)} of the form 
\begin{equation}
  \left\{
  \begin{aligned}
...
Since the \changed{existing} models \changed{considered} involve multiple parameters, a hybrid optimization framework \changed{is} implemented to \changed{find} the parameter set that minimizes the discrepancy between model predictions and experimental data.
...
A representative learning curve corresponding to the Det-MemODE architecture, consisting of two hidden layers with 128 neurons each, trained using the \verb|Adam| optimizer with a learning rate of \(10^{-2}\) and a batch size of 2, is presented in Figure~\ref{fig:learning-curve}.
The training process was conducted over \changed{360} epochs, with the \changed{objective} function $\mathcal{L}$ monitored on both the training and validation datasets to provide insights into convergence behavior and potential overfitting. The learning curve demonstrates a \changed{consistent decrease}
%ZG removed "and smooth"
in values of $\mathcal{L}$ for both datasets, which indicates effective optimization and \changed{a} satisfactory model fitting to the experimental data. The close agreement between the training and validation loss further confirms the generalization ability of the selected architecture.
...
\begin{figure}[!t]
  ...
  \caption{Learning curve for the Det-MemODE neural network architecture during training over 360 epochs.
...
The analysis \changed{encompasses the voltage waveform, the current waveform and the voltage-current hysteresis relationships}.
\changed{The voltage profile confirms the fidelity of the input stimulus. The temporal current response reveals the device's instantaneous electrical behavior and switching kinetics. The voltage-current hysteresis loops (\(\vm - \im\)) illustrate the fundamental memory properties that define memristive behavior.}
...
\subsection{Comparative Analysis with \changed{the MMS and GMMS Models}}

\noindent
To \changed{assess} the performance of the neural \changed{ODE} approach, a comprehensive comparative analysis is conducted between the developed neural network \changed{models} and the \changed{existing models of SDC memristors: the} Mean Metastable Switch (MMS) model and \changed{the} Generalized Mean Metastable Switch (GMMS) model.
...
\changed{Comparison results are plotted in Fig.~\ref{fig:comparison_mms}.} Quantitative analysis reveals substantial performance improvements achieved by the neural network approach. 
...
\begin{figure*}[!t]
  ...
  \caption{Comparative analysis of loss function values:
    (a) the Det-MemODE model, (b) the Dual-NN-MemODE model, (c) the MMS model, (d) the GMMS model. 
    \changed{Results for voltage amplitudes \(\Vs =0.5, 1.0, 1.5\rm \ V\) are plotted in red (\redbar{}), blue (\bluebar{}) and green (\greenbar{}), respectively}. 
...
Table~\ref{tab:mms_comp} presents a comparative analysis of the loss functions $\mathcal{L}_{\mathrm{base}}$ obtained for the \changed{proposed and existing} models, offering a quantitative assessment of their performance. The results highlight the neural network’s enhanced capability to reproduce complex and subtle aspects of memristor dynamics that are insufficiently captured by the phenomenological MMS/GMMS framework. For clarity, the highest loss value in each row is marked in red, whereas the lowest is marked in green. Overall, the neural network model consistently achieves substantially lower loss values, demonstrating superior accuracy and fidelity in representing the device behavior.
...
Table~\ref{tab:mms_comp_test} \changed{reports} the average loss function values $\mathcal{L}_{\mathrm{base}}$ calculated on the test dataset for the \changed{proposed and existing} models, providing a quantitative evaluation of their generalization performance across unseen memristor dynamics.
...
\TODO{What is the importance of optimizer type (mentioned in the search space). It would be interesting to know what are the obtained values of the loss function when using different values of certain hyperparameters (other parameters should be ones used for computations).}
